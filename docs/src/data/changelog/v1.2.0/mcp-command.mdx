...
The tools that run OpenTofu/Terraform take a `tf_path` argument, which accepts `tofu` or `terraform` and refuses a path, and `plan`, `apply`, and `destroy` take `parallelism` to cap how many units run at once.
