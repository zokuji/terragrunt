...
type renderConfigInput struct {
	...
	TFPath         string `json:"tf_path,omitempty"         jsonschema:"Which implementation fetches dependency outputs: tofu or terraform, found on the server's PATH. Overrides a unit's terraform_binary. Omit to use the server's default."`
...
func runRenderConfig(
	...
	applyToolTFPath(opts, input.TFPath)
