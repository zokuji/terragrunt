...
// ErrToolTFPathNotAllowed is returned when a tool call's tf_path names
// anything other than tofu or terraform.
var ErrToolTFPathNotAllowed = errors.New("tf_path must be tofu or terraform")

// toolTFPaths are the values a tool call's tf_path accepts, each a program
// name the server's PATH resolves. A path is refused so that an agent cannot
// choose which file runs.
var toolTFPaths = []string{"tofu", "terraform"}

// validateToolTFPath refuses a tool call's tf_path unless it is empty or one
// of [toolTFPaths].
func validateToolTFPath(tfPath string) error {
	if tfPath == "" || slices.Contains(toolTFPaths, tfPath) {
		return nil
	}

	return fmt.Errorf("%w, got %q", ErrToolTFPathNotAllowed, tfPath)
}

// applyToolTFPath points opts at the implementation a tool call's tf_path
// names, overriding the operator's --tf-path and a unit's terraform_binary the
// way the flag does on the CLI. An empty tf_path leaves opts alone. The value
// must already have passed [validateToolTFPath].
func applyToolTFPath(opts *options.TerragruntOptions, tfPath string) {
	if tfPath == "" {
		return
	}
