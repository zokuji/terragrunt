...
type runAllInput struct {
	...
	TFPath      string   `json:"tf_path,omitempty"     jsonschema:"Which implementation runs the units: tofu or terraform, found on the server's PATH. Overrides a unit's terraform_binary. Omit to use the server's default."`
...
func (in runAllInput) validate() error {
	...
	return validateToolTFPath(in.TFPath)
}
