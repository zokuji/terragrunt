...
// stand for, so the approved command line names the filters, binary, and
// parallelism the run uses. The arguments must already have passed
// [runAllInput.validate].
//
...
func approvalRunFlags(input runAllInput) string {
	...
	if input.TFPath != "" {
		b.WriteString(" --tf-path=" + input.TFPath)
	}
