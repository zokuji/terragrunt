...
type getOutputsInput struct {
	...
	TFPath           string `json:"tf_path,omitempty"           jsonschema:"Which implementation reads the outputs: tofu or terraform, found on the server's PATH. Overrides the unit's terraform_binary. Omit to use the server's default."`
...
func runGetOutputs(
	...
	if err := validateToolTFPath(input.TFPath); err != nil {
	...
	return outputsFetchViaStateRead(ctx, l, d, rootVenv, dir, input.TFPath)
}
