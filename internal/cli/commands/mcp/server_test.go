...
// TestAllowCmdRunsWithoutAsking pins the flag: a command matched up front runs
// with no prompt at all, which is what an operator wants once they know which
// commands a tree needs.
...
// TestAllowCmdMatchesAProgramTheTreeShipsByPath pins the hole a base-name
// allowlist leaves, and the pattern that closes it on purpose. A configuration
// that ships a file named after an allowed program and calls it by path would
// otherwise reach the real executor under a pattern naming only git. A pattern
// naming the path is the operator allowing that file.
...
// TestConfigurationCannotRunTerragruntsOwnPrograms pins the boundary
// --allow=exec draws now that it is drawn on the caller and not the program
// name: tofu, terraform, and git run when Terragrunt starts them, and a git a
// configuration names for itself is refused like any other program. An
// --allow-cmd pattern is the only way to let that named git run.
